...
<p align="center"> 
  <b>순수 C</b>로 작성한 초경량 Windows 카운트다운 및 포모도로 타이머 도구로, 깔끔하고 투명한 인터페이스를 제공하며,<br/>
  풍부한 사용자 정의 기능, 강력한 빠른 작업, 그리고 거의 시스템 리소스를 차지하지 않습니다.
</p>
...
<div align="center" style="margin-bottom: 30px;">
  ...
  와
  <a href="https://qm.qq.com/q/vjCduOQILI" target="_blank" style="text-decoration: none; font-weight: bold; color: #12B7F5;">QQ 그룹（1079222265）</a>
</div>


## 🎨 강력한 트레이 애니메이션

이제 트레이 아이콘이 “살아 움직일” 수 있습니다! Catime은 강력한 사용자 정의 애니메이션 기능을 제공합니다:

- **사용자 정의 애니메이션**: **GIF**, **WebP** 및 **PNG**, **JPG** 같은 정적 이미지를 트레이 아이콘으로 사용할 수 있습니다.
- **시스템 상태 표시**: **CPU 사용량**, **메모리 사용량**, **배터리 비율(%)** 과 **Caps Lock** 상태를 실시간으로 표시합니다.
...
> [!TIP]
> 더 재미있는 트레이 애니메이션을 찾고 계신가요? 함께하는 프로젝트 [MemeTray](https://github.com/MemeTray/MemeTray)를 확인해 보세요!



## 💫 [플러그인](https://github.com/vladelaina/Catime-Plugins)

> 플러그인은 **output.txt**에 내용을 쓰기만 하면 되고, Catime이 대신 표시합니다. 그게 전부입니다.
...
> [!TIP]
> 더 유용한 플러그인을 찾고 싶다면, 함께하는 프로젝트 [Catime-Plugins](https://github.com/vladelaina/Catime-Plugins)를 살펴보세요!



## ❤️‍🔥 계속 지켜봐 주세요

> **별을 남겨 주세요**. 순수 C로 만든 이 미니멀한 시간 도구가 계속 진화하는 모습을 확인하고, 향후 업데이트도 놓치지 마세요!
  ...
  <table align="center">
    <tr>
      ...
      <td style="vertical-align: middle;">
        무료 Windows 코드 서명 서비스는
        <a href="https://signpath.io">SignPath.io</a>에서 제공하며, 인증서는
        <a href="https://signpath.org/">SignPath Foundation</a>에서 후원합니다.
      </td>
    </tr>
  </table>
</div>
...
- GitHub Releases
  ...
  [GitHub Releases](https://github.com/vladelaina/Catime/releases/latest)에서 최신 버전을 다운로드하세요.
...
## [📑 사용 가이드](https://cati.me/guide)

- **트레이 아이콘 왼쪽 클릭**：**시간** 설정
- **트레이 아이콘 오른쪽 클릭**：**메뉴** 열기
- **편집 모드**：드래그로 위치 조정, 마우스 휠로 확대/축소, 🖱️ 오른쪽 클릭으로 편집 모드 종료

> 설정 파일은 `%LOCALAPPDATA%\Catime\config.ini`에 저장됩니다

## ⚡ [글꼴 최적화](https://vladelaina.github.io/Catime/tools/font-tool/)

공식 [글꼴 축소 도구](https://vladelaina.github.io/Catime/tools/font-tool/)를 사용하면 사용자 지정 글꼴 크기를 **90% 이상** 줄일 수 있습니다.

> Catime은 주로 숫자와 기호(`0-9`、`:`、`.`)를 표시하므로, 대부분의 글꼴 문자는 실제로 필요하지 않습니다.
>
> **권장**: 원본 글꼴 파일을 백업으로 보관해 주세요. 향후 소프트웨어 업데이트에서 더 많은 문자가 필요할 수 있습니다.

## 🛠️ 개발

**선행 조건**: Catime을 빌드하려면 MinGW와 CMake가 필요합니다.
...
# Windows - 아래 설정 설명을 참고하세요
```

<details>
<summary>📋 Windows 환경 설정 설명</summary>

### 📦 필수 도구

| 도구 | 설명 | 권장 버전 형식 | 다운로드 링크 |
| ---------- | ---------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------- |
...
#### 1. MinGW 설치
1. [MinGW Build](https://github.com/niXman/mingw-builds-binaries/releases/latest)에서 `x86_64-<version>-release-win32-seh-ucrt-rtv<num>-rev<num>.7z` 다운로드
2. `C:\mingw64`로 압축 해제
3. `C:\mingw64\bin`을 시스템 `PATH` 환경 변수에 추가:
   - 열기: `제어판 → 시스템 → 고급 시스템 설정 → 환경 변수`
   - **시스템 변수**에서 `Path`를 찾아 "편집"을 클릭
   - 추가: `C:\mingw64\bin`

#### 2. CMake 설치
1. [CMake](https://cmake.org/download/)에서 `cmake-<version>-windows-x86_64.msi`를 다운로드하고 실행
2. 설치 과정에서 "Add CMake to the system PATH"를 선택
...
# 🎁 [Catime 지원](https://cati.me/support)
...
## [✨ 크레딧](https://cati.me/#thanks)

훌륭한 아이콘 디자인을 제공해 준 [猫屋敷梨梨Official](https://space.bilibili.com/26087398) 에게 특별히 감사드립니다.

> 다음 기여자들에게 시간 순으로 특별히 감사드립니다：
...
아이콘 저작권 고지：
- 소프트웨어 아이콘은 [猫屋敷梨梨Official](https://space.bilibili.com/26087398) 이(가) 제작했으며, 모든 권리를 보유합니다. 원저작자의 명시적 승인 없이 어떠한 상업적 목적에도 사용할 수 없습니다.
글꼴 라이선스：
- MIT：
...
배경화면：
- 작성자：[猫屋敷梨梨Official](https://space.bilibili.com/26087398)
  - 동적 배경화면 링크：[猫屋敷梨梨「별이 있는 곳」일러스트(밤 버전)](https://steamcommunity.com/sharedfiles/filedetails/?id=3171487185) 및 [猫屋敷梨梨-「별이 있는 곳」일러스트(황혼 버전)](https://steamcommunity.com/sharedfiles/filedetails/?id=3171483657)
---
<div align="center">
