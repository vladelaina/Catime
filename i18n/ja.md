...
# v1.5.0 が公開されました！🎉 [今すぐダウンロード](https://github.com/vladelaina/Catime/releases/latest) - `995KB のみ`！
</div>
...
<p align="center"> 
  <b>純粋なC</b> で作られた、超軽量なWindowsのカウントダウン＆ポモドーロタイマー。きれいで透明感のあるインターフェース、<br/>
  豊富なカスタマイズ性、強力なクイック操作、そしてほとんどシステムリソースを消費しません。
</p>
...
<div align="center" style="margin-bottom: 30px;">
  ...
  と
  <a href="https://qm.qq.com/q/vjCduOQILI" target="_blank" style="text-decoration: none; font-weight: bold; color: #12B7F5;">QQ グループ（1079222265）</a>
</div>


## 🎨 強力なトレイアニメーション

トレイアイコンが今、「生き生き」と動き出します！Catimeは強力なカスタムアニメーション機能を提供します：

- **カスタムアニメーション**：**GIF**、**WebP**、および **PNG**、**JPG** などの静的画像をトレイアイコンとして使用できます。
- **システム状態の表示**：**CPU使用率**、**メモリ使用量**、**バッテリー残量（％）**、そして **Caps Lock** の状態をリアルタイム表示します。
...
> [!TIP]
> もっと面白いトレイアニメーションを探していますか？関連プロジェクト [MemeTray](https://github.com/MemeTray/MemeTray) をぜひチェックしてみてください！



## 💫 [プラグイン](https://github.com/vladelaina/Catime-Plugins)

> プラグインは **output.txt** に内容を書き込むだけ。Catimeが表示してくれます。これだけです。
...
> [!TIP]
> もっと便利なプラグインを探すなら、関連プロジェクト [Catime-Plugins](https://github.com/vladelaina/Catime-Plugins) をチェックしてみてください！



## ❤️‍🔥 私たちをフォローし続けよう

> **スターをお願いします**。純粋なCで作られたミニマルな時間ツールが、進化し続ける様子をぜひ見守ってください。今後のアップデートをお見逃しなく！
  ...
  <table align="center">
    <tr>
      ...
      <td style="vertical-align: middle;">
        無料のWindowsコード署名サービスは
        <a href="https://signpath.io">SignPath.io</a>が提供し、証明書は
        <a href="https://signpath.org/">SignPath Foundation</a>がスポンサーしています。
      </td>
    </tr>
  </table>
</div>
...
- GitHub Releases
  ...
  [GitHub Releases](https://github.com/vladelaina/Catime/releases/latest) から最新バージョンをダウンロードしてください。
...
## [📑 使い方ガイド](https://cati.me/guide)

- **トレイアイコンの左クリック**：**時間** を設定
- **トレイアイコンの右クリック**：**メニュー** を開く
- **編集モード**：ドラッグで位置を調整、マウスホイールでズーム、🖱️ の右クリックで編集モードを終了

> 設定ファイルは `%LOCALAPPDATA%\Catime\config.ini` に保存されます

## ⚡ [フォント最適化](https://vladelaina.github.io/Catime/tools/font-tool/)

公式の [フォント圧縮ツール](https://vladelaina.github.io/Catime/tools/font-tool/) を使うことで、カスタムフォントのサイズを **90% 以上** 縮小できます。

> Catimeは主に数字と記号（`0-9`、`:`、`.`）を表示するため、ほとんどのフォント文字は実際には必要ありません。
>
> **おすすめ**：バックアップとして元のフォントファイルを残しておいてください。今後のソフトウェア更新で、より多くの文字が必要になる可能性があります。

## 🛠️ 開発

**前提条件**：CatimeのビルドにはMinGWとCMakeが必要です。
...
<details>
<summary>📋 Windows環境の設定説明</summary>

### 📦 必須ツール

| ツール | 説明 | 推奨バージョン形式 | ダウンロードリンク |
| ---------- | ---------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| **MinGW**  | GCC コンパイラ | `x86_64-<version>-release-win32-seh-ucrt-rtv<num>-rev<num>.7z` | [MinGW Build](https://github.com/niXman/mingw-builds-binaries/releases/latest) |
| **CMake**  | ビルドツール | `cmake-<version>-windows-x86_64.msi` | [CMake](https://cmake.org/download/) |

### 🔧 インストール手順

#### 1. MinGWをインストール
1. [MinGW Build](https://github.com/niXman/mingw-builds-binaries/releases/latest) から `x86_64-<version>-release-win32-seh-ucrt-rtv<num>-rev<num>.7z` をダウンロード
2. `C:\mingw64` に解凍
3. `C:\mingw64\bin` をシステムの `PATH` 環境変数に追加：
   - 開く：`コントロール パネル → システム → システムの詳細設定 → 環境変数`
   - **システム変数** の中から `Path` を見つけて「編集」
   - 追加：`C:\mingw64\bin`

#### 2. CMakeをインストール
1. [CMake](https://cmake.org/download/) から `cmake-<version>-windows-x86_64.msi` をダウンロードして実行
2. インストール中に「Add CMake to the system PATH」を選択
...
# 🎁 [Catimeをサポート](https://cati.me/support)
...
## [✨ クレジット](https://cati.me/#thanks)

特別に感謝します [猫屋敷梨梨Official](https://space.bilibili.com/26087398) による素晴らしいアイコンデザイン。

> 以下の貢献者に特別に感謝します（時系列順）：
...
## 🗝️ 著作権に関する説明
本プロジェクトはオープンソースの Apache License 2.0 のもとで公開されています。

アイコンの著作権に関する説明：
- ソフトウェアアイコンは [猫屋敷梨梨Official](https://space.bilibili.com/26087398) によって作成され、すべての権利が保留されています。元の作者の明確な許可なく、いかなる商用目的にも使用してはなりません。
フォントライセンス：
- MIT：
...
- 作者：[猫屋敷梨梨Official](https://space.bilibili.com/26087398)
  - 動的壁紙リンク：[猫屋敷梨梨「星の在処」イラスト（夜版）](https://steamcommunity.com/sharedfiles/filedetails/?id=3171487185) と [猫屋敷梨梨-「星の在処」イラスト（黄昏版）](https://steamcommunity.com/sharedfiles/filedetails/?id=3171483657)
---
<div align="center">
