...
static DialogMarkdownState* GetPluginMarkdown(HWND hwndDlg) {
    return hwndDlg ? (DialogMarkdownState*)GetWindowLongPtrW( hwndDlg, GWLP_USERDATA)
                   : NULL;
}
...
static INT_PTR CALLBACK PluginSecurityDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_INITDIALOG: {
            ...
            SetWindowPos(GetDlgItem(hwndDlg, IDC_PLUGIN_SECURITY_CANCEL_BTN), NULL, cancelX, btnY, cancelWidth, btnHeight, SWP_NOZORDER);
            SetWindowPos(GetDlgItem(hwndDlg, IDC_PLUGIN_SECURITY_RUN_ONCE_BTN), NULL, runOnceX, btnY, runOnceWidth, btnHeight, SWP_NOZORDER);
            SetWindowPos(GetDlgItem(hwndDlg, IDC_PLUGIN_SECURITY_TRUST_BTN), NULL, trustX, btnY, trustWidth, btnHeight, SWP_NOZORDER);
            ...
            if (!Utf8ToWide(GetPendingPluginPath(), pluginPathWide, _countof(pluginPathWide))) {
            ...
            int written = _snwprintf_s(messageWide, PLUGIN_SECURITY_MESSAGE_BUFFER_CHARS, _TRUNCATE, L"<md>\n" L"%s\n\n" L"%s\n" L"`%ls`\n\n" L"%s\n" L"- %s\n" L"- %s\n" L"- %s (%s)\n" L"- %s\n" L"- %s\n\n" L"%s\n" L"- %s\n" L"- %s\n" L"- %s\n" L"- %s\n" L"</md>", GetLocalizedString(NULL, L"PluginSecTitle"), GetLocalizedString(NULL, L"PluginSecLocation"), pluginPathWide, GetLocalizedString(NULL, L"PluginSecWhatToKnow"), GetLocalizedString(NULL, L"PluginSecOutputRule"), GetLocalizedString(NULL, L"PluginSecPermDefault"), GetLocalizedString(NULL, L"PluginSecPermAdmin"), GetLocalizedString(NULL, L"PluginSecUACPrompt"), GetLocalizedString(NULL, L"PluginSecScriptFile"), GetLocalizedString(NULL, L"PluginSecReview"), GetLocalizedString(NULL, L"PluginSecTips"), GetLocalizedString(NULL, L"PluginSecTip1"), GetLocalizedString(NULL, L"PluginSecTip2"), GetLocalizedString(NULL, L"PluginSecTip3"), GetLocalizedString(NULL, L"PluginSecTip4") );
            ...
            switch (LOWORD(wParam)) {
                ...
                case IDC_PLUGIN_SECURITY_TEXT:
                    if (HIWORD(wParam) == STN_CLICKED) {
                        ...
                        if (DialogMarkdown_HandleClick( GetPluginMarkdown(hwndDlg), pt)) {
                            return TRUE;
                        }
                    }
                    return TRUE;
        ...
        case WM_DRAWITEM: {
            ...
            if (lpDrawItem->CtlID == IDC_PLUGIN_SECURITY_TEXT) {
                ...
                DialogModern_DrawRoundedRect( hdc, &panelRect, DialogModern_Scale(DialogModern_GetDpi(hwndDlg), 14), palette.field, palette.border, 1);
                DialogMarkdownState* markdown = GetPluginMarkdown(hwndDlg);
                if (markdown) {
                    ...
                    DialogMarkdown_Render(markdown, hdc, drawRect, palette.accent, palette.text);
                    if (hOldFont) {
                        SelectObject(hdc, hOldFont);
                    }
                    if (oldBkMode != 0) {
