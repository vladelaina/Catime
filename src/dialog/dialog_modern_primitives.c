...
UINT DialogModern_GetDpi(HWND hwnd) {
    ...
    if (hwnd && user32) {
        ...
        CATIME_LOAD_PROC_ADDRESS(user32, "GetDpiForWindow", getDpiForWindowFunc);
...
void DialogModern_SetChildRect96(HWND hwndDlg, int controlId, UINT dpi, int x, int y, int width, int height) {
    HWND control = hwndDlg ? GetDlgItem(hwndDlg, controlId) : NULL;
    if (!control) return;
    SetWindowPos(control, NULL, DialogModern_Scale(dpi, x), DialogModern_Scale(dpi, y), DialogModern_Scale(dpi, width), DialogModern_Scale(dpi, height), SWP_NOZORDER | SWP_NOACTIVATE);
}
BOOL DialogModern_GetChildRect96(HWND hwndDlg, int controlId, UINT dpi, RECT* rect) {
...
BOOL DialogModern_MeasureText96(HWND hwnd, HFONT font, const wchar_t* text, UINT dpi, SIZE* size) {
    ...
    SIZE physical = {0};
    BOOL measured = GetTextExtentPoint32W( hdc, text, (int)wcslen(text), &physical);
...
HFONT DialogModern_CreateFont(UINT dpi, int pixelSize, LONG weight) {
    ...
    if (FAILED(StringCchCopyW(lf.lfFaceName, _countof(lf.lfFaceName), L"Segoe UI"))) {
        return NULL;
    }
    return CreateFontIndirectW(&lf);
}
void DialogModern_DrawRoundedRect(HDC hdc, const RECT* rect, int cornerDiameter, COLORREF fill, COLORREF border, int borderWidth) {
    if (!hdc || !rect) return;
    ...
    RoundRect(hdc, rect->left, rect->top, rect->right, rect->bottom, cornerDiameter, cornerDiameter);
    if (oldPen) SelectObject(hdc, oldPen);
    if (oldBrush) SelectObject(hdc, oldBrush);
    if (borderWidth > 0 && pen) DeleteObject(pen);
    if (brush) DeleteObject(brush);
}
void DialogModern_DrawCloseButton(HDC hdc, const RECT* rect, UINT dpi, BOOL hovered, BOOL focused, BOOL highContrast, COLORREF accent, COLORREF mutedText) {
    if (!hdc || !rect) return;
...
void DialogModern_DrawText(HDC hdc, HFONT font, COLORREF color, const RECT* rect, const wchar_t* text, UINT format) {
...
static COLORREF DialogModernBlendColor(COLORREF from, COLORREF to, int toPercent) {
    if (toPercent < 0) toPercent = 0;
    if (toPercent > 100) toPercent = 100;
    int fromPercent = 100 - toPercent;
    return RGB((GetRValue(from) * fromPercent + GetRValue(to) * toPercent) / 100, (GetGValue(from) * fromPercent + GetGValue(to) * toPercent) / 100, (GetBValue(from) * fromPercent + GetBValue(to) * toPercent) / 100);
}
static void DialogModernDrawBezierStroke(HDC hdc, const POINT* points, DWORD pointCount, int width, COLORREF color) {
    ...
    brush.lbColor = color;
    HPEN pen = ExtCreatePen(PS_GEOMETRIC | PS_SOLID | PS_ENDCAP_ROUND | PS_JOIN_ROUND, (DWORD)(width > 0 ? width : 1), &brush, 0, NULL);
...
static void DialogModernScaleSignaturePoints(const POINT* source, POINT* target, DWORD pointCount, int originX, int originY, int scale) {
    for (DWORD i = 0; i < pointCount; i++) {
        target[i].x = (source[i].x - originX) * scale;
        target[i].y = (source[i].y - originY) * scale;
    }
}
void DialogModern_DrawTitleSignature(HDC hdc, const RECT* titleRect, UINT dpi, int titleTextWidth, COLORREF accent, COLORREF surface, BOOL darkMode, BOOL highContrast) {
    ...
    COLORREF leading = highContrast ? accent :
        DialogModernBlendColor(accent, RGB(0xA8, 0xEC, 0xFF), darkMode ? 38 : 58);
    ...
    HBITMAP sampleBitmap = sampleDc ? CreateCompatibleBitmap( hdc, outputWidth * sampleScale, outputHeight * sampleScale) : NULL;
    ...
    if (sampleBitmap) {
        ...
        DialogModernScaleSignaturePoints(stroke, scaledStroke, _countof(stroke), bounds.left, bounds.top, sampleScale);
        DialogModernScaleSignaturePoints( airStroke, scaledAirStroke, _countof(airStroke), bounds.left, bounds.top, sampleScale);
        drawStroke = scaledStroke;
        drawAirStroke = scaledAirStroke;
        drawScale = sampleScale;
    }
    if (!highContrast) {
        DialogModernDrawBezierStroke( drawDc, drawAirStroke, _countof(airStroke), DialogModern_Scale(dpi, 2) * drawScale, glow);
        DialogModernDrawBezierStroke(drawDc, drawStroke, _countof(stroke), softWidth * drawScale, glow);
        DialogModernDrawBezierStroke(drawDc, drawStroke, 4, mainWidth * drawScale, leading);
        DialogModernDrawBezierStroke(drawDc, drawStroke + 3, 7, mainWidth * drawScale, accent);
    } else {
        DialogModernDrawBezierStroke(drawDc, drawStroke, _countof(stroke), mainWidth * drawScale, accent);
    }
    if (sampleBitmap) {
        int oldMode = SetStretchBltMode(hdc, HALFTONE);
        POINT oldOrigin = {0};
        SetBrushOrgEx(hdc, bounds.left, bounds.top, &oldOrigin);
        StretchBlt(hdc, bounds.left, bounds.top, outputWidth, outputHeight, sampleDc, 0, 0, outputWidth * sampleScale, outputHeight * sampleScale, SRCCOPY);
