...
#endif
#define CMD_COLOR_OPTIONS_END (CMD_COLOR_OPTIONS_BASE + MAX_COLOR_OPTIONS - 1)
#if ((CLOCK_IDM_TEXT_EFFECT_BASE <= CMD_COLOR_OPTIONS_END && CLOCK_IDM_TEXT_EFFECT_END >= CMD_COLOR_OPTIONS_BASE) || \
     (CLOCK_IDC_FONT_ADVANCED >= CMD_COLOR_OPTIONS_BASE && CLOCK_IDC_FONT_ADVANCED <= CMD_COLOR_OPTIONS_END) || \
     (CLOCK_IDC_FONT_LICENSE_AGREE >= CMD_COLOR_OPTIONS_BASE && CLOCK_IDC_FONT_LICENSE_AGREE <= CMD_COLOR_OPTIONS_END))
#error "Static menu IDs overlap dynamic color menu command range"
#endif
...
static BOOL AppendColorOptionCopyTo(PredefinedColor** options, size_t* count, const char* color) {
...
static BOOL PaletteContainsColor(PredefinedColor* options, size_t count, const char* color) {
...
static BOOL AppendColorOptionTo(PredefinedColor** options, size_t* count, const char* color) {
...
static BOOL AppendNormalizedGradientSegment(const char* begin, const char* end, char* outValue, size_t outSize, size_t* used, BOOL firstSegment) {
...
BOOL NormalizeGradientConfigValue(const char* colorInput, char* outValue, size_t outSize) {
    ...
    for (const char* p = colorInput;; p++) {
        ...
        if (!AppendNormalizedGradientSegment(segmentBegin, p, outValue, outSize, &used, colorCount == 0)) {
            return FALSE;
        }
        colorCount++;
        if (*p == '\0') {
...
BOOL BuildColorOptionsFromConfigValue(const char* colorOptions, PredefinedColor** outOptions, size_t* outCount) {
    ...
    while (token) {
        ...
        if (*token) {
            if (token[0] != '#') {
                ...
                if (written >= 0 && (size_t)written < sizeof(colorWithHash) && !AppendColorOptionTo(&options, &count, colorWithHash)) {
...
static BOOL AppendColorOptionConfigValue(char* outValue, size_t outSize, const char* color, BOOL* firstColor) {
...
BOOL BuildColorOptionsConfigValueFromArray(PredefinedColor* options, size_t count, char* outValue, size_t outSize) {
    if (!outValue || outSize == 0) return FALSE;
    outValue[0] = '\0';
    BOOL firstColor = TRUE;
    for (size_t i = 0; i < count; i++) {
        const char* color = options[i].hexColor;
        if (color && !IsGradientColorString(color) && !AppendColorOptionConfigValue(outValue, outSize, color, &firstColor)) {
            return FALSE;
        }
    }
    for (size_t i = 0; i < count; i++) {
