/**
 * @file plugin_ipc.c
 * @brief Plugin IPC implementation
 */
...
BOOL PluginIPC_HandleMessage(HWND hwnd, PCOPYDATASTRUCT pcds) {
    ...
    MultiByteToWideChar(CP_UTF8, 0, (const char*)pcds->lpData,
                       pcds->cbData, g_pluginDisplayText, 128);
    g_pluginDisplayText[127] = L'\0';
    ...
    LOG_INFO("Plugin message received (type: 0x%04X, size: %lu bytes): %ls",
             pluginType, pcds->cbData, g_pluginDisplayText);
