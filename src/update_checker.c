 ...
 */
 ...
 /**
  * @brief Extract pre-release identifier from version string
  * @param version Version string (e.g., "1.3.0-alpha2")
  * @param preRelease Output buffer for pre-release string
  * @param maxLen Maximum length of output buffer
  * @return TRUE if pre-release found, FALSE if stable release
  */
 ...
 /**
  * @brief Compare pre-release identifiers according to semantic versioning rules
  * @param pre1 First pre-release string (e.g., "alpha2", "beta1", "rc3")
  * @param pre2 Second pre-release string
  * @return 1 if pre1 > pre2, -1 if pre1 < pre2, 0 if equal
  * 
  * Semantic Versioning 2.0.0 rules:
  * - Stable release (no pre-release) > any pre-release
  * - Pre-release precedence: alpha < beta < rc < stable
  * - Same type: compare numeric suffix (alpha2 < alpha10)
  */
 static int ComparePreRelease(const char* pre1, const char* pre2) {
     /** Both stable releases (no pre-release) */
     if (!pre1[0] && !pre2[0]) return 0;
     
     /** One is stable, one is pre-release: stable > pre-release */
     if (!pre1[0]) return 1;   /** version1 is stable, version2 is pre-release */
     if (!pre2[0]) return -1;  /** version1 is pre-release, version2 is stable */
     
     /** Both are pre-releases: compare type and number */
     
     /** Define pre-release type priority: alpha < beta < rc */
     int type1 = 0, type2 = 0;
     int num1 = 0, num2 = 0;
     
     if (strncmp(pre1, "alpha", 5) == 0) {
         type1 = 1;
         sscanf(pre1 + 5, "%d", &num1);
     } else if (strncmp(pre1, "beta", 4) == 0) {
         type1 = 2;
         sscanf(pre1 + 4, "%d", &num1);
     } else if (strncmp(pre1, "rc", 2) == 0) {
         type1 = 3;
         sscanf(pre1 + 2, "%d", &num1);
     }
     
     if (strncmp(pre2, "alpha", 5) == 0) {
         type2 = 1;
         sscanf(pre2 + 5, "%d", &num2);
     } else if (strncmp(pre2, "beta", 4) == 0) {
         type2 = 2;
         sscanf(pre2 + 4, "%d", &num2);
     } else if (strncmp(pre2, "rc", 2) == 0) {
         type2 = 3;
         sscanf(pre2 + 2, "%d", &num2);
     }
     
     /** Compare pre-release type first */
     if (type1 != type2) {
 ...
 /**
  * @brief Compare two semantic version strings with pre-release support
  * @param version1 First version string to compare (e.g., "1.3.0-alpha2")
  * @param version2 Second version string to compare (e.g., "1.3.0")
  * @return 1 if version1 > version2, -1 if version1 < version2, 0 if equal
  * 
  * Supports Semantic Versioning 2.0.0:
  * - Format: MAJOR.MINOR.PATCH[-PRERELEASE]
  * - Examples:
  *   - "1.3.0-alpha2" < "1.3.0-beta1" < "1.3.0-rc1" < "1.3.0"
  *   - "1.3.0-alpha2" < "1.3.0-alpha10"
  *   - "1.2.9" < "1.3.0-alpha1" < "1.3.0"
  */
 int CompareVersions(const char* version1, const char* version2) {
     ...
     /** Parse major.minor.patch numbers (stops at '-' if present) */
     ...
     /** Major.minor.patch are equal: compare pre-release identifiers */
